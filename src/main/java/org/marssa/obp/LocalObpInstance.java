...
import org.marssa.obp.dummy.DummyRadar;
import org.marssa.obp.dummy.RandomDummyInstrument;
...
public class LocalObpInstance extends BasicIdentified implements ObpInstance {
    ...
    private Instrument dummyOutdoorWeatherStation() {
        ...
        attributes.put(AttributeNames.TEMPERATURE, RandomDummyInstrument.DoubleRange.create(-10.0, 30.4));
        attributes.put(AttributeNames.ATMOSPHERIC_PRESSURE, RandomDummyInstrument.DoubleRange.create(99800.0, 10200.0));
        attributes.put(AttributeNames.WIND_SPEED, RandomDummyInstrument.DoubleRange.create(0.0, 30.0));
        return new RandomDummyInstrument(getUuid(), "Hurricane H102","dummy outdoor weather station",attributes);
    }

    private Instrument dummyIndoorWeatherStation() {
        Map<String, RandomDummyInstrument.DoubleRange> attributes = new HashMap<>();
        attributes.put(AttributeNames.TEMPERATURE, RandomDummyInstrument.DoubleRange.create(18.0, 25.0));
        attributes.put(AttributeNames.RELATIVE_HUMIDITY, RandomDummyInstrument.DoubleRange.create(50.0, 100.0));
        return new RandomDummyInstrument(getUuid(), "Typhoon T1000","dummy indoor weather station",attributes);
    }
